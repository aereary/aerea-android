...
test("keeps the schedule separate, restyles the extended month, and removes statistics", () => {
  ...
  assert.match(cssSource, /var\(--event-color\) 16%/);
...
test("offers a persisted Little aérea simplified calendar-only screen", () => {
  ...
  assert.match(cssSource, /\.unified-calendar-cell \.calendar-cell-event[\s\S]*text-align: center/);
  assert.match(cssSource, /\.app-shell:has\(\.unified-month-grid\)[\s\S]*\.bottom-nav[\s\S]*display: none !important/);
